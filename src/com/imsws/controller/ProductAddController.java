...
import com.imsws.dao.ProductAddDao;
import com.imsws.dto.ProductAddDto;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (formItems != null && formItems.size() > 0) {
					for(FileItem formItem: formItems) {
						if(!formItem.isFormField()) {
							...
							ProductAddDao pad=new ProductAddDao();
							count=pad.doUpload(list);
				...
				if(count>0) {
					request.setAttribute("msg", "Product Upload Successfully");
